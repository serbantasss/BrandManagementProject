...
class ProductFilterPage():
    def __init__(self,master,connection,user,univbg,univfg):
    ...
    def getPLACEMENT_TYPE(self, type):
        ...
        for x in records:
            ret.append(x[0])
        return ret
    def createlist(self,lista:List,opt):
        if opt == 1:
            ret = ','
        else:
            ret = ''
    ...
    def createquery(self):
        ...
        fmm = fmm + f"AND\n Products.LOCATION_TYPE in ({self.createlist(self.selectionLocation, 1)})\n"
        ...
        if self.selectionLocation.CheckVar[2].get() == True:
            fmm = fmm + self.createlist(self.selectionHipermaket, (self.selectionLocation.CheckVar[1].get() == True))
